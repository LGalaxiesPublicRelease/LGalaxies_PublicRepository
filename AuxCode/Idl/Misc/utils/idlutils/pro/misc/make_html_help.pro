...
;
 ...
 PRINTF,final_file,'<PRE>     ? make_html_help</PRE><P>'
 ...
 FOR i=0,count-1 DO BEGIN
  ...
  IF (entry.nline GT 0) THEN BEGIN
   ...
   ENDIF ELSE BEGIN
    ...
    IF (LinkCode) THEN $
      PRINTF,final_file,'<STRONG>(See <A HREF="file:'+fname+'">'+fname+'</A>)</STRONG><P>' $
    ELSE  $
...
PRO make_html_help, sources, outfile, VERBOSE=verbose, TITLE=title, STRICT=strict, LINK_FILES=LinkFiles
 ...
 IF (NOT keyword_set(LinkFiles)) THEN LinkFiles = 0
 infiles = ''
 istlb = 0b
