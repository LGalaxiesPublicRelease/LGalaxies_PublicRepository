...
/* compare weights for sort */ 
int weight_compare(const void *vi0, const void *vi1)
{
  int i0, i1;
  i0=*((int *) vi0);
  i1=*((int *) vi1);
  if(weight[i0]<weight[i1]) return(-1);
  if(weight[i0]>weight[i1]) return(1);
  return(0);
}
...
{
#define WARNMAX		8
  int dnadj, i, j, unified, nadj, pass, istart, iend;
  ...
  do {
    ...
    /* sort the polygons by weight */
    isort=(int *) malloc((*npoly)*sizeof(int));
    weight=(double *) malloc((*npoly)*sizeof(double));
    for(i=0;i<*npoly;i++) isort[i]=i;
    for(i=0;i<*npoly;i++) 
      if(!poly[i]) 
        weight[i]=-1.;
      else 
        weight[i]=poly[i]->weight;
    qsort((void *) isort, *npoly, sizeof(int), weight_compare);

    /* for each group with a unique weight ... */
    dnadj = 0;
    for(istart=0; istart<*npoly;) {
      for(iend=istart; iend<*npoly && 
            weight[isort[iend]]==weight[isort[istart]]; iend++);
      iend--;

      /* printf("weight_start=%e weight_end=%e\n", 
         weight[isort[istart]], weight[isort[iend]]); */
      if(weight[isort[istart]]!=-1.) {
        /* ... try unifying each polygon in turn ... */
        for (i = istart; i<=iend ; i++) {
          if (!poly[isort[i]]) continue;
          
          /* ... with another polygon */
          for (j = i+1; j <=iend; j++) {
            if (!poly[isort[j]]) continue;
            /* only unify polygons with the same weight */
            if (poly[isort[i]]->weight != poly[isort[j]]->weight) continue;

            /* if applying old ids, then only unify polygons with the same id */
            if (fmt.newid == 'o' && poly[isort[i]]->id != poly[isort[j]]->id) 
              continue;
            
            /* try unifying polygons */
            unified = unify_poly(&poly[isort[i]], poly[isort[j]]);
            if (unified == -1) {
              if (WARNMAX/2 > 0 && dnadj > 0 && dnadj <= WARNMAX/2) msg("\n");
              fprintf(stderr, "unify_poly: failed to unify polygons %d & %d; continuing\n", poly[isort[i]]->id, poly[isort[j]]->id);
              continue;
            }
            ...
            if (unified) {
              ...
              if (dnadj < WARNMAX/2) {
                msg(" (%d %d)", poly[isort[i]]->id, poly[isort[j]]->id);
              } else if (dnadj == WARNMAX/2) {
                msg(" ... more\n");
              }
              free_poly(poly[isort[j]]);
              poly[isort[j]] = 0x0;
              dnadj++;
            }
          }
    ...
    free(isort); isort=0x0;
    free(weight); weight=0x0;
  } while (dnadj);
