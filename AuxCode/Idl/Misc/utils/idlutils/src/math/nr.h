...
#define NR_SUCCESS_VALUE 0
#define NR_FAIL_VALUE 1
#define NR_FAILURE(text) \
 {printf("NR ERROR: "); printf(text); printf("\n"); return NR_FAIL_VALUE;}
...
int isort(unsigned long n, int arr[]); /* Added by DJS 19jul95 */
...
int lfit(float x[], float y[], float sig[], int ndat, float a[], int ia[],
...
void lubksb0(float **a, int n, int *indx, float b[]);
...
/* Conflicts with /usr/include/sys/select.h on a Linux box (DJS 07-Jul-1999)
...
void simpr(float y[], float dydx[], float dfdx[], float **dfdy,
	...
	void (*derivs)(float, float [], float []));
int sindexx(int n, float arr[], int indx[]);
