  ...
  for (mbin=0; mbin<massbins; ++mbin)
  {
  	for (j=0; j<NFofsInSample[snap]; ++j)
  	{
  				...
  				MCMC_FOF2[j].M_Crit200[snap]<minfofmass+(mbin+1)*(maxfofmass-minfofmass)/(double)massbins)
  		{
  			...
  			if (MCMC_GAL[HashTable[i]].log10_StellarMass[snap]>masslimit_low && MCMC_GAL[HashTable[i]].log10_StellarMass[snap]<=masslimit_high)
  				ncen=1;

  			for (k=1; k<MCMC_GAL[HashTable[i]].ngal[snap]; ++k)
  			{
  				if (MCMC_GAL[HashTable[i+k]].log10_StellarMass[snap]>masslimit_low && MCMC_GAL[HashTable[i+k]].log10_StellarMass[snap]<=masslimit_high)
  					nsat++;
  			} //for
  			usedbymass[mbin]++;
  		}
